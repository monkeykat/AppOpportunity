...
Phase 2 is independent from `1_Scout`. It reads only the Phase 2 SQLite database during normal execution.
...
Before each investigation, Phase 2 reads Phase 1's
`1_Scout/src/app_scout.db` and synchronizes new or changed opportunity data into
...
Each run also writes a timestamped copy of its console output to `2_Investigator/logs/` while continuing to display progress in the terminal.
...
Continuous mode syncs `1_Scout` before every iteration, waits 30 seconds between
...
Each continuous iteration creates its own timestamped log in `2_Investigator/logs/`.
